...
- `talk/` — only what is specific to this talk: `Slide` wrappers, the deck's order, talk-only tweaks done by subclassing a package scene.
...
- `uv run manim -ql src/aionanim/scenes/<file>.py <Scene>` — run from the project root so `manim.cfg` applies; `-qh` for the talk. Deck: `uv run manim-slides render -q h talk/aion_slides.py` (`-q h`, not `-qh`: manim-slides reads the `h` as `--help`).
